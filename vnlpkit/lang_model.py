...
def load_lang_model(path_w2v, path_swem):
    ...
    from utils.load_and_dump import load_with_cloudpickle, dump_with_cloudpickle
...
def create_columns_from_nested_array(df, col):
    return df.join(pd.DataFrame(
        df[col].tolist(), index=df.index).add_prefix(col + '_'))
...
def vectorize_with_swem(lang_model, df, text_cols, method='max'):
    ...
    for col in tqdm(text_cols):
        df_vec = pd.DataFrame(df[col].apply(
            lambda x: lang_model.infer_vector(x, method=method)))
