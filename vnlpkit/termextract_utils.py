...
def calc_term_imp(
    frequency, metric: str = "score_pp", average_rate: float = 1.0, lr_mode: int = 1
):
    if metric == "score_pp":
...
def extract_kw_by_termextract(
    ...
    if morph_analyzer == "janome":
        ...
        wakati = MeCab.Tagger() if dic is None else MeCab.Tagger(f"-d {dic}")
    ...
    if type(metrics) == str:
        term_imp = calc_term_imp(
            frequency, metrics, average_rate=average_rate, lr_mode=lr_mode
        )
    ...
    output = {cmp_noun: value for cmp_noun, value in data_collection.most_common()}
    if type(upper) == int:
        if upper > len(output):
            upper = len(output)
